...
def download_csv_mpIIdataset():
    ...
    if os.path.exists(train_to_path) and getmd5(train_to_path) == TRAIN_MD5:
        print("[GazeLib] Train csv has downloaded and passed md5 cheking.")
    else:
        print("[GazeLib] Start downloading {}.".format(TRAIN_CSV_URL))
        if _download_url_to_path(train_to_path, TRAIN_CSV_URL):
            if getmd5(train_to_path) == TRAIN_MD5:
                print("[GazeLib] Download training csv at {} succeeds!".format(train_to_path))
            else:
                os.remove(train_to_path)
                print("[GazeLib] Train.csv not passes MD5 verification. Try again!")
        else:
            print("[GazeLib] Download training csv failed.")

    if os.path.exists(test_to_path) and getmd5(test_to_path) == TEST_MD5:
        print("[GazeLib] Test csv has downloaded and passed md5 cheking.")
    else:
        print("[GazeLib] Start downloading {}.".format(TEST_CSV_URL))
        if _download_url_to_path(test_to_path, TEST_CSV_URL):
            if getmd5(test_to_path) == TEST_MD5:
                print("[GazeLib] Download test csv at {} succeeds!".format(test_to_path))
            else:
                os.remove(test_to_path)
                print("[GazeLib] Test.csv not passes MD5 verification. Try again!")
        else:
            print("[GazeLib] Download test csv failed.")
...
def download_demo_Img():
    ...
    if os.path.exists(demo_Img_path) and getmd5(demo_Img_path) == DEMO_IM_MD5:
        print("[GazeLib] Demo image has downloaded and passed md5 cheking.")
    else:
        print("[GazeLib] Start downloading {}.".format(DEMO_IM_URL))
        if _download_url_to_path(demo_Img_path, DEMO_IM_URL):
            if getmd5(demo_Img_path) == DEMO_IM_MD5:
                print("[GazeLib] Download demo image at {} succeeds!".format(demo_Img_path))
            else:
                os.remove(demo_Img_path)
                print("[GazeLib] Demo image not passes MD5 verification. Try again!")
        else:
            print("[GazeLib] Download demo image failed.")
